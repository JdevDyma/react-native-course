...
export default function Favorites({ navigation }) {
  ...
  const data = favoritesShoesIds
    .map((id) => catalog.find((shoe) => shoe.id === id))
    .filter(Boolean);
  ...
  const renderItem = ({ item }) => (
    <View style={[styles.cardContainer, { flex: 1 / numColumns }]}>
      <VerticalCard item={item} listScreen isFavorite
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyListContainer}>
            <TextBoldL style={styles.emptyText}>Vous n'avez pas encore de favoris</TextBoldL>
          </View>
        }
