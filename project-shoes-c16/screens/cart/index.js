...
export default function Cart() {
  ...
  const removeShoesFromCart = (id) => profile.writeProfile((user) => ({ cart: removeCartLine(user.cart, id) }));
  const updateQuantity = (id, increase) => profile.writeProfile((user) => ({ cart: changeCartQuantity(user.cart, id, increase) }));
  ...
  return (
    <View style={[styles.container, { paddingLeft: insets.left, paddingRight: insets.right }]}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={{ flex: 1 / numColumns }}><ListItem item={item} removeShoesFromCart={removeShoesFromCart}
            updateQuantity={updateQuantity} busy={!profile.canWrite} /></View>
        )}
        ...
        ListFooterComponent={profile.user ? <PriceSummary totalAmount={totalAmount} /> : null}
