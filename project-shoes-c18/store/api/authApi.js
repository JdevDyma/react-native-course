  ...
  endpoints: (builder) => ({
    sign: builder.mutation({
      async queryFn(values, api, extraOptions, baseQuery) {
        ...
        if (!Number.isSafeInteger(generation) || api.getState().auth.generation !== generation || api.signal.aborted) return failure();
        ...
        try {
          ...
          if (api.signal.aborted || api.getState().auth.generation !== generation) return failure();
