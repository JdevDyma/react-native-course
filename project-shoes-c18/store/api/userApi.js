...
function photoUrlOrEmpty(value) {
  if (value == null || value === "") return "";
  if (typeof value !== "string" || value.length > 4096) throw new Error("Adresse de photo invalide.");
  const url = new URL(value);
  if (url.protocol !== "https:" || url.username || url.password) throw new Error("Adresse de photo invalide.");
  return value;
}
...
function normalizeProfile(value, id) {
  ...
  return {
    ...
    photoUrl: photoUrlOrEmpty(value.photoUrl),
...
function validatePatch(argument) {
  ...
  if (!keys.length || keys.some((key) => !["email", "favoritesIds", "seenNotifsIds", "cart", "fullName", "location", "photoUrl"].includes(key))) {
  ...
  for (const key of keys) {
    if (key === "email") {
      ...
      result.photoUrl = photoUrlOrEmpty(patch.photoUrl);
  ...
  endpoints: (builder) => ({
    uploadUserPicture: builder.mutation({
      async queryFn(argument, api) {
        try { return { data: await uploadProfilePicture(argument, api) }; }
        catch { return failure("L’envoi de la photo n’a pas été confirmé. Votre sélection est conservée."); }
      },
    }),
...
export const { useUploadUserPictureMutation, useGetUserByIdQuery, useCreateUserMutation, useUpdateUserMutation } = userApi;
