...
export default function ListItem({ item, navigateToDetails, isSeen = false, updateNotif, busy = false }) {
  const canNavigate = typeof navigateToDetails === "function" && !busy;
  const image = item.items?.[0]?.image;

  function navigate() {
    if (!canNavigate) return;
    navigateToDetails(item.id);
    if (!isSeen && typeof updateNotif === "function") void updateNotif(item.id);
  }
  return (
    <View style={styles.container}>
      <Touchable color={colors.BLUE} disabled={busy}
